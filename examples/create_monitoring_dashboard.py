            ...
            },
            {
                ...
                'query': f'''
                    SELECT 
                        ...
                        AVG(UNIX_TIMESTAMP(resolved_at) - UNIX_TIMESTAMP(timestamp)) / 60 as avg_resolution_time_minutes
                    FROM system_alerts
                    WHERE timestamp >= '{last_24h}'
            ...
            },
            {
                ...
                'query': f'''
                    SELECT 
                        ...
                        cpu_cores_used,
                        cpu_cores_total,
                        ROUND(cpu_cores_used * 100.0 / cpu_cores_total, 2) as cpu_utilization_percent,
                        memory_gb_used,
                        memory_gb_total,
                        ROUND(memory_gb_used * 100.0 / memory_gb_total, 2) as memory_utilization_percent,
                        disk_gb_used,
                        disk_gb_total,
                ...
                'query': f'''
                    WITH sla_metrics AS (
                        SELECT 
                            ...
                            DATE(timestamp) as date,
                            COUNT(*) as total_requests,
                            SUM(CASE WHEN response_time_ms <= 500 THEN 1 ELSE 0 END) as requests_under_500ms,
                    ...
                    SELECT 
                        ...
                        date,
                        total_requests,
                        ROUND(requests_under_500ms * 100.0 / total_requests, 2) as performance_sla_percent,
                        ROUND(successful_requests * 100.0 / total_requests, 2) as availability_sla_percent,
                        avg_response_time,
                        CASE 
...
- **Resolution Time:** < 15 minutes
    ...
    def create_sla_widgets(self) -> List[Dict[str, Any]]:
        ...
        sla_widgets = [
            ...
            },
            {
                ...
                'params': {
                    'dataset_name': 'SLA Compliance',
                    'columns': ['service_name', 'environment', 'performance_sla_percent', 'availability_sla_percent', 'sla_status'],
    ...
    def _create_widgets(self, widget_configs: List[Dict], widget_category: str) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                if widget_result['success']:
                    ...
                    if add_result['success']:
                        ...
                        print(f"   ✅ {widget_category.title()} widget added: {params.get('title', 'Untitled')}")
...
def main():
    ...
    try:
        ...
        alert_results = builder.create_alert_widgets()
        performance_results = builder.create_performance_widgets()
        error_results = builder.create_error_widgets()
        resource_results = builder.create_resource_widgets()
        sla_results = builder.create_sla_widgets()

...
def load_dashboard_tools(mcp_server):
  ...
  def add_widget_to_dashboard_impl(dashboard_id: str, widget_spec: dict, dataset_name: str = None, dataset_query: str = None) -> dict:
    ...
    try:
      widget_spec = {
        ...
        'layers': [
          {
            'mark': 'bar',
            'encoding': {'y': {'field': bar_field, 'type': 'quantitative'}}
          },
          {
            'mark': 'line',
            'encoding': {'y': {'field': line_field, 'type': 'quantitative'}}
          }
        ],
  ...
  def reposition_widget(
    ...
    try:
      ...
      if position['x'] < 0 or position['y'] < 0:
        return {
          'success': False,
          'error': 'Position x and y must be non-negative'
        }
      
      if position['width'] <= 0 or position['height'] <= 0:
        return {
          'success': False,
          'error': 'Position width and height must be positive'
        }
      
      if position['x'] + position['width'] > 12:
        return {
          'success': False,
          'error': 'Widget extends beyond 12-column grid (x + width > 12)'
        }

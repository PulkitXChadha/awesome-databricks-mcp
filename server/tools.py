...
def load_tools(mcp_server):
  ...
  @mcp_server.tool
  def build_dlt_medallion_pipeline(
    tables: list[dict],
    pipeline_name: str,
    catalog: str,
    ...
    """Build a Delta Live Tables (DLT) pipeline with medallion architecture from source tables.
    
    This tool creates a comprehensive DLT pipeline configuration with:
    - Bronze layer: Raw data ingestion with basic validation
    - Silver layer: Cleaned and transformed data with data quality rules
    ...
    Args:
        tables: List of table definitions, each containing:
                - name: Table name
                - source_path: Data source path or table reference
                - source_type: 'parquet', 'delta', 'json', 'csv', 'kafka', 'autoloader'
                - primary_key: List of primary key columns (optional)
                - business_keys: List of business key columns for deduplication (optional)
                - description: Table description (optional)
        pipeline_name: Name for the DLT pipeline
        catalog: Unity Catalog name for the pipeline
        bronze_schema: Schema name for bronze tables (default: 'bronze')
        silver_schema: Schema name for silver tables (default: 'silver')
        gold_schema: Schema name for gold tables (default: 'gold')
        target_database_name: Target database name (optional, defaults to catalog)
        storage_location: Storage location for pipeline artifacts (optional)
        
    Returns:
        Dictionary with DLT pipeline configuration and SQL code
    """
    try:
      if not tables:
        return {'success': False, 'error': 'At least one table definition is required'}
        
      if not pipeline_name or not catalog:
        return {'success': False, 'error': 'pipeline_name and catalog are required'}
        
      target_db = target_database_name or catalog
      ...
      pipeline_config = {
        ...
        'catalog': catalog,
        'target': target_db,
        'storage': storage_location or f's3://your-bucket/dlt/{pipeline_name}/',
        'configuration': {
        ...
        'libraries': [{
          'file': {
            'path': f'/Workspace/pipelines/{pipeline_name}_pipeline.py'
          }
        }]
      }
      
      # Generate Python DLT code
      dlt_code = f'''
import dlt
...
"""
Bronze tables contain raw data with minimal processing.
...
'''
      ...
      # Generate Bronze layer tables
      for table in tables:
        table_name = table['name']
        source_path = table['source_path']
        source_type = table.get('source_type', 'parquet').lower()
        description = table.get('description', f'Bronze table for {table_name}')
        
        bronze_table_name = f"{bronze_schema}_{table_name}"
        bronze_tables.append(bronze_table_name)
        
        if source_type == 'autoloader':
          dlt_code += f'''
@dlt.table(
  name="{bronze_table_name}",
  comment="{description} - Raw data with audit columns",
  table_properties={{
    "quality": "bronze",
...
def {bronze_table_name}():
  return (
    spark.readStream
    .format("cloudFiles")
    .option("cloudFiles.format", "parquet")  # Adjust format as needed
    .option("cloudFiles.schemaEvolution", "true")
    .option("cloudFiles.schemaHints", "id long, created_date timestamp")
    .load("{source_path}")
    .withColumn("_ingestion_timestamp", F.current_timestamp())
    .withColumn("_source_file", F.input_file_name())
    .withColumn("_pipeline_run_id", F.lit(spark.conf.get("spark.databricks.clusterUsageTags.clusterOwnerOrgId")))
  )
'''
        elif source_type in ['parquet', 'delta', 'json', 'csv']:
          dlt_code += f'''
@dlt.table(
  name="{bronze_table_name}",
  comment="{description} - Raw data with audit columns",
...
@dlt.table(
  name="{bronze_table_name}",
  comment="{description} - Raw Kafka data with audit columns",
  table_properties={{
    "quality": "bronze",
    "pipelines.autoOptimize.managed": "true"
  }}
...
def {bronze_table_name}():
  return (
    spark.readStream
    .format("kafka")
    .option("kafka.bootstrap.servers", "{source_path}")  # Kafka broker
    .option("subscribe", "{table_name}_topic")  # Adjust topic name
    .load()
...
"""
Silver tables contain cleaned, validated data with:
- Data quality rules and constraints
- Deduplication based on business keys
- Type conversions and standardization
- SCD Type 2 for historical tracking (where applicable)
"""
...
'''
      ...
      for table in tables:
        table_name = table['name']
        bronze_table_name = f"{bronze_schema}_{table_name}"
        silver_table_name = f"{silver_schema}_{table_name}"
        silver_tables.append(silver_table_name)
        
        primary_key = table.get('primary_key', [])
        business_keys = table.get('business_keys', primary_key)
        description = table.get('description', f'Silver table for {table_name}')
        
        # Build data quality expectations
        expectations = []
        if primary_key:
          expectations.append(f'@dlt.expect("valid_primary_key", "{" AND ".join([f"{pk} IS NOT NULL" for pk in primary_key])}")')
        expectations.append('@dlt.expect("recent_data", "_ingestion_timestamp >= current_date() - interval 7 days")')
        expectations.append('@dlt.expect_or_drop("no_nulls_in_key_fields", "TRUE")  # Customize based on your key fields')
...
@dlt.table(
  ...
  comment="{description} - Cleaned and validated data",
  table_properties={{
    "quality": "silver",
    "pipelines.autoOptimize.managed": "true",
...
def {silver_table_name}():
  ...
  # Remove system columns for processing
  data_columns = [col for col in df.columns if not col.startswith('_')]
  df_clean = df.select(*data_columns, "_ingestion_timestamp")
...
'''
        ...
        if business_keys:
          dlt_code += f'''  
  # Deduplication based on business keys: {", ".join(business_keys)}
  window_spec = Window.partitionBy({", ".join([f'"{key}"' for key in business_keys])}).orderBy(F.desc("_ingestion_timestamp"))
  df_clean = df_clean.withColumn("row_number", F.row_number().over(window_spec)).filter(F.col("row_number") == 1).drop("row_number")
'''
...
'''
      ...
      # Generate Gold layer tables with business aggregations
      dlt_code += f'''

# Gold Layer - Business-Ready Data
"""
Gold tables contain business-ready, aggregated data:
...
'''
      ...
      # Generate a sample gold table that aggregates data
      if len(tables) > 0:
        sample_table = tables[0]['name']
        gold_summary_table = f"{gold_schema}_{sample_table}_summary"
        gold_tables.append(gold_summary_table)
        
        dlt_code += f'''
@dlt.table(
  name="{gold_summary_table}",
  comment="Daily summary metrics for {sample_table}",
  table_properties={{
    "quality": "gold",
    "pipelines.autoOptimize.managed": "true",
    "delta.autoOptimize.optimizeWrite": "true"
...
def {gold_summary_table}():
  return (
    dlt.read("{silver_schema}_{sample_table}")
    .groupBy(F.date_trunc("day", F.col("updated_timestamp")).alias("date"))
    .agg(
      F.count("*").alias("record_count"),
      F.countDistinct("*").alias("unique_records"),  # Adjust based on actual key columns
      F.max("updated_timestamp").alias("last_updated"),
      F.min("_ingestion_timestamp").alias("first_ingested")
    )
    .withColumn("created_timestamp", F.current_timestamp())
  )
'''
        
      # Generate additional gold tables for each source table
      for table in tables:
        table_name = table['name']
        gold_table_name = f"{gold_schema}_{table_name}_daily"
        if gold_table_name not in gold_tables:
          gold_tables.append(gold_table_name)
          
          dlt_code += f'''
@dlt.table(
  name="{gold_table_name}",
  comment="Daily business metrics for {table_name}",
  table_properties={{
    "quality": "gold",
    "pipelines.autoOptimize.managed": "true"
  }}
...
def {gold_table_name}():
  return (
    dlt.read("{silver_schema}_{table_name}")
    .filter(F.col("is_active") == True)
    .withColumn("business_date", F.date_trunc("day", F.col("updated_timestamp")))
    .groupBy("business_date")
    .agg(
      F.count("*").alias("daily_record_count"),
      F.approx_count_distinct("*").alias("approx_unique_records"),  # Adjust key column
      F.max("updated_timestamp").alias("max_update_time")
    )
...
"""
Additional data quality checks and monitoring
"""

@dlt.table(
  name="data_quality_metrics",
  comment="Data quality metrics across all pipeline tables"
)
def data_quality_metrics():
  """Aggregate data quality metrics from all layers"""
  # This would typically combine quality metrics from all tables
  # Implementation depends on your specific quality requirements
  return spark.sql("""
    SELECT 
      current_timestamp() as metric_timestamp,
      'pipeline_health' as metric_type,
      '{pipeline_name}' as pipeline_name,
      'active' as status
  """)
'''

      # Add import statement for Window functions if business keys are present
      if any(table.get('business_keys') or table.get('primary_key') for table in tables):
        dlt_code = dlt_code.replace('from datetime import datetime', 
      ...
      result = {
        ...
        'recommendations': [
          'Review and customize data quality expectations based on your specific requirements',
          'Adjust column names and data types in the generated code to match your source data',
          'Configure appropriate storage location and access permissions',
          'Set up monitoring and alerting for pipeline failures',
          'Consider partitioning strategies for large tables',
          'Implement proper error handling and dead letter queues for streaming data',
          'Set up data retention policies according to your compliance requirements'
        ],
        'next_steps': [
          f'1. Save the generated DLT code to /Workspace/pipelines/{pipeline_name}_pipeline.py',
          '2. Create the DLT pipeline using the Databricks UI or CLI with the provided configuration',
          '3. Test the pipeline with a subset of data first',
          '4. Monitor the pipeline execution and adjust data quality rules as needed',
          '5. Set up downstream consumption patterns for gold tables'
        ]
      }
      ...
      print(f'❌ Error building DLT pipeline: {str(e)}')
      return {'success': False, 'error': f'Error: {str(e)}'}

...
class TestToolIntegration:
    ...
    @pytest.mark.integration
    async def test_core_tools_loading(self, mcp_server, mock_env_vars):
        """Test core tools loading."""
        load_core_tools(mcp_server)
        
        tools = await mcp_server.get_tools()
        core_tool_names = ['health', 'get_workspace_info', 'get_user_info']
        
        for tool_name in core_tool_names:
            assert tool_name in tools, f"Core tool {tool_name} not loaded"
    
    @pytest.mark.integration
    async def test_unity_catalog_tools_loading(self, mcp_server, mock_env_vars):
        """Test Unity Catalog tools loading."""
        load_uc_tools(mcp_server)
        
        tools = await mcp_server.get_tools()
        uc_tool_names = ['list_uc_catalogs', 'describe_uc_catalog', 'list_uc_schemas']
        
        for tool_name in uc_tool_names:
            assert tool_name in tools, f"Unity Catalog tool {tool_name} not loaded"
    
    @pytest.mark.integration
    async def test_sql_tools_loading(self, mcp_server, mock_env_vars):
        """Test SQL tools loading."""
        load_sql_tools(mcp_server)
        
        tools = await mcp_server.get_tools()
        sql_tool_names = ['list_warehouses', 'execute_dbsql', 'get_warehouse_status']
        
        for tool_name in sql_tool_names:
            assert tool_name in tools, f"SQL tool {tool_name} not loaded"
    
    @pytest.mark.integration
    async def test_jobs_tools_loading(self, mcp_server, mock_env_vars):
        """Test Jobs and Pipelines tools loading."""
        load_job_tools(mcp_server)
        
        tools = await mcp_server.get_tools()
        jobs_tool_names = ['list_jobs', 'list_pipelines', 'run_job_now']
        
        for tool_name in jobs_tool_names:
            assert tool_name in tools, f"Jobs tool {tool_name} not loaded"
    
    @pytest.mark.integration
    async def test_dashboard_tools_loading(self, mcp_server, mock_env_vars):
    ...
    @pytest.mark.integration
    async def test_data_management_tools_loading(self, mcp_server, mock_env_vars):
        """Test Data Management tools loading."""
        load_data_tools(mcp_server)
        
        tools = await mcp_server.get_tools()
        data_tool_names = ['list_dbfs_files', 'get_dbfs_file_info', 'list_volumes']
        
        for tool_name in data_tool_names:
            assert tool_name in tools, f"Data Management tool {tool_name} not loaded"
    
    @pytest.mark.integration
    async def test_governance_tools_loading(self, mcp_server, mock_env_vars):
    ...
    @pytest.mark.integration
    async def test_tool_functionality_integration(self, mcp_server, mock_env_vars):
    ...
    @pytest.mark.integration
    async def test_tool_registration_consistency(self, mcp_server, mock_env_vars):
        """Test that tool registration is consistent across loads."""
        # Load tools twice
        load_tools(mcp_server)
        first_tools = await mcp_server.get_tools()
        first_count = len(first_tools)
        
        # Clear and reload
        mcp_server._tool_manager._tools.clear()
        load_tools(mcp_server)
        second_tools = await mcp_server.get_tools()

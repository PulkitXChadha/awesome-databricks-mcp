...
class TestDashboardPerformance:
    ...
    def test_widget_rendering_with_large_dataset(self, mcp_server, mock_env_vars):
        ...
        with patch('server.tools.dashboards.WorkspaceClient') as mock_client:
            ...
            large_dataset = [
                [f'row_{i}', f'value_{i}', f'category_{i % 10}', i * 100.5]
    ...
    def test_memory_usage_monitoring(self, mcp_server, mock_env_vars):
        ...
        with patch('server.tools.dashboards.WorkspaceClient') as mock_client:
            ...
            assert memory_increase < 100, f"Memory increased by {memory_increase:.2f}MB, exceeding 100MB limit"
...
class TestWidgetPerformance:
    ...
    def test_widget_creation_batch_performance(self, mcp_server, mock_env_vars):
        ...
        with patch('server.tools.dashboards.WorkspaceClient') as mock_client:
            ...
            assert avg_time_per_widget < 0.5, f"Average time per widget {avg_time_per_widget:.2f}s exceeds 0.5s"
    ...
    def test_widget_layout_algorithm_performance(self, mcp_server, mock_env_vars):
        ...
        with patch('server.tools.dashboards.WorkspaceClient') as mock_client:
            ...
            for layout_type in layout_types:
                ...
                result = layout_tool.fn(
                    dashboard_id='layout-test-dashboard',
...
class TestQueryPerformance:
    ...
    def test_query_execution_timeout_enforcement(self, mcp_server, mock_env_vars):
        ...
        with patch('server.tools.dashboards.WorkspaceClient') as mock_client:
            ...
            test_tool = mcp_server._tool_manager._tools['test_dataset_query']
            result = test_tool.fn(
                query='SELECT * FROM huge_table ORDER BY random_column',
                warehouse_id='warehouse-123'
            )
...
class TestStressTest:
    ...
    def test_dashboard_widget_limit_stress(self, mcp_server, mock_env_vars):
        ...
        with patch('server.tools.dashboards.WorkspaceClient') as mock_client:
            ...
            assert successful_additions <= 100, f"System allowed {successful_additions} widgets, exceeding expected limit"
            assert successful_additions > 90, f"System only allowed {successful_additions} widgets, below expected capacity"
    ...
    def test_concurrent_dashboard_modifications(self, mcp_server, mock_env_vars):
        ...
        with patch('server.tools.dashboards.WorkspaceClient') as mock_client:
            ...
            assert success_rate > 0.7, f"Success rate {success_rate:.2f} too low for concurrent operations"
    ...
    def test_memory_stress_large_datasets(self, mcp_server, mock_env_vars):
        ...
        with patch('server.tools.dashboards.WorkspaceClient') as mock_client:
            ...
            for i in range(5):
                result = test_tool.fn(
                    query=f'SELECT * FROM huge_table_{i}',
                    warehouse_id='warehouse-123',
                    limit=1000
                )
                ...
                assert memory_increase < 200, f"Memory increased by {memory_increase:.2f}MB, exceeding 200MB limit"

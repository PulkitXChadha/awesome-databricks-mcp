...
def load_uc_tools(mcp_server):
  ...
  def describe_uc_schema(
    ...
    try:
      ...
      return {
        ...
        'message': (
          f'Schema {catalog_name}.{schema_name} details retrieved successfully '
          f'with {len(table_list)} table(s)'
        ),
      }
  ...
  def describe_uc_table(table_name: str, include_lineage: bool = False) -> dict:
    ...
    try:
      ...
      table_info = {
        ...
        'storage_location': (
          table.storage_location if hasattr(table, 'storage_location') else None
        ),
      }
  ...
  def list_uc_volumes(catalog_name: str, schema_name: str) -> dict:
    ...
    try:
      # Initialize Databricks SDK
      # Note: WorkspaceClient initialization would go here in production
      # w = WorkspaceClient(
  ...
  # ========================================================================
  # UNITY CATALOG VOLUME RESOURCES
  # ========================================================================
  # These resources enable direct read access to volume contents via MCP

  @mcp_server.resource('volumes://{catalog}.{schema}/list')
  def list_volumes_resource(catalog: str, schema: str) -> dict:
    """Resource to list all volumes in a Unity Catalog schema.

    Provides read-only access to volume listings for MCP clients.
    ...
    Returns:
        Dictionary with volume listings formatted for resource consumption
    """
    try:
      # Initialize Databricks SDK
      w = WorkspaceClient(
      ...
      for volume in volumes:
        volume_list.append(
          {
            'name': volume.name,
            'full_name': f'{catalog}.{schema}.{volume.name}',
            'volume_type': volume.volume_type,
            'comment': volume.comment,
            'owner': volume.owner,
            'created_at': volume.created_at,
      ...
      return {
        ...
        'volumes': volume_list,
        'count': len(volume_list),
        'resource_type': 'volume_listing',
        'timestamp': None,  # Could add current timestamp if needed
      }

    except Exception as e:
      raise ResourceError(f'Unable to list volumes in {catalog}.{schema}: {str(e)}')

  @mcp_server.resource('volumes://{catalog}.{schema}.{volume}/list')
  def list_volume_contents_resource(catalog: str, schema: str, volume: str) -> dict:
    """Resource to list contents of a Unity Catalog volume.

    Provides directory listing of files and folders in the volume root.
    ...
    Returns:
        Dictionary with volume contents listing
    """
    try:
      # Initialize Databricks SDK
      w = WorkspaceClient(
      ...
      # List files and directories in volume
      try:
        files_list = w.files.list(volume_path)
          ...
          contents.append(
            {
              ...
              'modified_time': getattr(file_info, 'modification_time', None),
            }
          )
        ...
        return {
          ...
          'contents': contents,
          'count': len(contents),
          'resource_type': 'volume_contents',
        }

      except Exception as api_error:
        # Handle case where Files API might not be available or volume is empty
        return {
          'volume': f'{catalog}.{schema}.{volume}',
          'path': '/',
          'contents': [],
          'count': 0,
          'resource_type': 'volume_contents',
          'note': f'Volume exists but contents not accessible via Files API: {str(api_error)}',
        }

    except Exception as e:
      raise ResourceError(
        f'Unable to access volume contents for {catalog}.{schema}.{volume}: {str(e)}'
      )

  @mcp_server.resource(
    'volumes://{catalog}.{schema}.{volume}/files/{filepath*}', mime_type='text/plain'
  )
  def read_volume_file_resource(catalog: str, schema: str, volume: str, filepath: str) -> str:
    """Resource to read file content from a Unity Catalog volume.

    Provides direct file access within volumes for MCP clients.

    Args:
        catalog: Name of the catalog
        schema: Name of the schema
        volume: Name of the volume
        filepath: Path to the file within the volume (supports nested paths)

    Returns:
        File content as string (for text files)

    Raises:
        ResourceError: If file cannot be read or doesn't exist
    """
    try:
      # Initialize Databricks SDK
      w = WorkspaceClient(
      ...
      full_path = f'/Volumes/{catalog}/{schema}/{volume}/{clean_filepath}'

      # Security check: ensure the path doesn't contain directory traversal
      if '..' in filepath or filepath.startswith('/'):
        raise ResourceError('Invalid file path: directory traversal not allowed')

      # Read file content
      try:
        # Use Files API to download file content
        file_content = w.files.download(full_path)

        # For now, return as text. In production, you might want to:
        # 1. Detect file type and set appropriate MIME type
        # 2. Handle binary files differently
        # 3. Implement size limits

        if isinstance(file_content, bytes):
          # Try to decode as UTF-8, fallback to latin-1 for binary
          try:
            return file_content.decode('utf-8')
          except UnicodeDecodeError:
            # For binary files, you might want to return base64 encoded content
            import base64

            return (
              f'[Binary file - base64 encoded]\n{base64.b64encode(file_content).decode("ascii")}'
            )
        else:
        ...
        raise ResourceError(
          f"File '{clean_filepath}' not found or not accessible: {str(api_error)}"
        )
      ...
      raise ResourceError(
        f'Unable to read file {filepath} from volume {catalog}.{schema}.{volume}: {str(e)}'
      )

  @mcp_server.resource('volumes://{catalog}.{schema}.{volume}/dirs/{dirpath*}')
  def list_volume_directory_resource(catalog: str, schema: str, volume: str, dirpath: str) -> dict:
    """Resource to list contents of a specific directory within a Unity Catalog volume.

    Provides directory navigation within volumes for MCP clients.

    Args:
        catalog: Name of the catalog
        schema: Name of the schema
        volume: Name of the volume
        dirpath: Path to the directory within the volume (supports nested paths)

    Returns:
        Dictionary with directory contents listing
    """
    try:
      # Initialize Databricks SDK
      w = WorkspaceClient(
      ...
      # Remove leading slash if present and ensure trailing slash
      clean_dirpath = dirpath.strip('/')
      if clean_dirpath:
        full_path = f'/Volumes/{catalog}/{schema}/{volume}/{clean_dirpath}/'
        display_path = f'/{clean_dirpath}/'
      else:
        full_path = f'/Volumes/{catalog}/{schema}/{volume}/'
        display_path = '/'

      # Security check: ensure the path doesn't contain directory traversal
      if '..' in dirpath:
        raise ResourceError('Invalid directory path: directory traversal not allowed')

      # List directory contents
      try:
        files_list = w.files.list(full_path)
        ...
        raise ResourceError(
          f"Directory '{clean_dirpath}' not found or not accessible: {str(api_error)}"
        )
      ...
      raise ResourceError(
        f'Unable to list directory {dirpath} in volume {catalog}.{schema}.{volume}: {str(e)}'
      )

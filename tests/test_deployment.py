      ...
      async with http_session.get(url, timeout=10) as response:
        if response.status != expected_status:
          failed_endpoints.append(
            {'endpoint': endpoint, 'expected': expected_status, 'actual': response.status}
          )
  ...
  if failed_endpoints:
    ...
    for failure in failed_endpoints:
      if 'error' in failure:
        ...
        failure_msg += (
          f'  - {failure["endpoint"]}: Expected {failure["expected"]}, got {failure["actual"]}\n'
        )
...
async def test_smoke_tool_execution(deployment_url, http_session):
  ...
  payload = {'tool': 'health', 'parameters': {}}

  async with http_session.post(tool_url, json=payload, timeout=15) as response:
    if response.status == 404:
  ...
  async with http_session.get(invalid_url) as response:
    ...
    if response.content_type == 'application/json':
      ...
      assert 'detail' in error_data or 'error' in error_data, (
        'Error response missing detail/error field'
      )

  # Test invalid tool execution
  tool_url = f'{deployment_url}/mcp/execute'
  invalid_payload = {'tool': 'nonexistent_tool', 'parameters': {}}

  async with http_session.post(tool_url, json=invalid_payload) as response:
    if response.status != 404:  # Endpoint exists
      assert response.status in [400, 422], (
        f'Expected 400/422 for invalid tool, got {response.status}'
      )
  ...
  async with http_session.options(test_url, headers=headers) as response:
    ...
    if response.status == 200:
      ...
      assert (
        ...
        or 'access-control-allow-methods' in cors_headers
...
async def test_concurrent_requests(deployment_url, http_session):
  ...
  assert successful >= num_requests * 0.95, (
    f'Too many failed requests: {num_requests - successful}/{num_requests}'
  )
...
async def test_unauthorized_access(deployment_url, http_session):
  ...
  protected_endpoints = ['/api/v1/admin', '/api/v1/user/profile', '/mcp/execute/sensitive_tool']
...
async def test_metrics_endpoint(deployment_url, http_session):
  ...
  async with http_session.get(metrics_url) as response:
    ...
    assert response.status in [200, 401], (
      f'Metrics endpoint returned unexpected status: {response.status}'
    )
...
def test_deployment_configuration():
  ...
  if deployment_url:
    ...
    required_vars = ['DATABRICKS_HOST', 'DATABRICKS_APP_NAME']
